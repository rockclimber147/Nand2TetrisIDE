...
export abstract class JackVisitorTopLevel<T> extends VisitorBase<T> {
  public override visit(node: ASTNode): T {
    switch (node.kind) {
      case ASTNodeKind.CLASS:      return this.visitClass(node as JackAST.JackClassNode);
      case ASTNodeKind.SUBROUTINE: return this.visitSubroutine(node as JackAST.JackSubroutineNode);
      default:
...
export abstract class JackVisitorAll<T> extends JackVisitorTopLevel<T> {
  public override visit(node: ASTNode): T {
    switch (node.kind) {
      case ASTNodeKind.STATEMENT:  return this.visitStatement(node as JackAST.JackStatementNode);
      case ASTNodeKind.EXPRESSION: 
      case ASTNodeKind.TERM:        return this.visitExpression(node as JackAST.JackExpressionNode);
