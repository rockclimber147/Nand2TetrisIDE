...
export class GenericTokenizer {
  ...
  constructor(private source: string, private spec: [RegExp, TokenType][]) {}
  ...
  tokenize(): Token[] {
    ...
    while (this.cursor < this.source.length) {
      ...
      if (!matched) {
        throw new Error(`Unexpected character at line ${this.line}, col ${this.column}: ${this.source[this.cursor]}`);
      }
    }
    return tokens;
  }
